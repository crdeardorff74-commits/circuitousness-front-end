...
const Tutorial = (function () {
    ...
    //
    // ⚠ THE BOARD IS UNPLAYABLE FOR AS LONG AS A BEAT RUNS, and that is
    // not a tuning knob — it is what borrowing the singleton Maze costs.
    // A tip that carries a beat therefore presents itself as a MODAL:
    // centered, backdropped, unmistakably a thing you dismiss (user call
    // 2026-08-11). The earlier corner placement was inherited from the
    // text-only tips, which block nothing; on a beat tip it made a card
    // that had quietly taken the game away look like a card you could
    // ignore. Text-only tips still use the corner — see styles.css
    // #tooltipCard.tooltip-demo.
    //
